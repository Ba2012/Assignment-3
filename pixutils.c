...
void pixMap_sort(pixMap *p) {
 ...
 BMP16_map* pixMap_32_to_16(pixMap *p){
	 ...
	 BMP16_map *p16 = BMP16_map_init(p->height, p->width, 0, 5, 6, 5);
	 //loop through the array of arrays
	 uint16_t bit16;
	 for(int i = 0; i< p->height; i++) {
		 for(int j =0; j< p->width; j++) {			 
			 uint16_t r = p->pixArray[i][j].r >> 3;
			 uint16_t g = p->pixArray[i][j].g >> 2;
			 uint16_t b = p->pixArray[i][j].b >> 3;
			 //fprintf(stdout, "Red: %d\n Green: %d\n Blue: %d\n", p16->Rbits, p16->Gbits, p16->Bbits );			 
			 //printf("PRINTING BIT16");
			 bit16 = ((r) <<11) | ((g) << 5)| (b);			 
			 p16->pixArray[(p->height-1) - i][j] = bit16;
